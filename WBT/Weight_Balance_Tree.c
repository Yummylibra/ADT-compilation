...
int main(){
    ...
    while(choice != 8){
        ...
        printf("8 - delete WBT and exit\n");
		printf("9 - split an WBT tree\n");
		printf("10 - union 2 WBT trees\n");
        ...
        if(choice == 0){
			...
			if(rank == NOTIN){
				...
				printf("the rank of the node with key %d is: %d\n",number, rank);
			}
			printf("\n");
		}
		else if(choice == 7){
			...
			if(t == NULL){
				...
				printf("Node founded, address:%p, key:%d, size:%d\n",(void*)t, t->key, t->size);
			}
			printf("\n");
		}
			...
			//split avltree
			int array[10] = {60, 20, 80, 10, 30, 70, 115, 35, 100, 120};
			int x = 75;
			Node* Tsplit = NULL;
			for(int i = 0; i < 10; i ++){
			...
			printf("Please inter a valid selection number.\n");
		}
		choice = -1;
    }
    delete_WBT(root);
