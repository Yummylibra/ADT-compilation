 ...
 *
 * Student Name (Last name, First name): Zheyuan, Wei
 ...
 *
 * Sign here with your name: Zheyuan Wei
 *
 *
 */
...
void insertCastMember(char title[MAX_STR_LEN], char studio[MAX_STR_LEN], int year, ReviewNode *head, char name[MAX_STR_LEN], float salary)
{
    ...
    while (p != NULL)
    {
            ...
            p->review.year == year)
        {
            ...
            if (q == NULL)
            { // there is no end (no begin. nothing. its void. il est noir)
            ...
            // no return : Since duplicates are allowed this time;
        }
        p = p->next;
    }
}
